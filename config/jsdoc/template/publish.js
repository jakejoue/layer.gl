...
function generate(title, docs, filename, resolveLinks, showTitle = true) {
    let docData;
    let html;
    let outpath;
    ...
    if (members.namespaces.length) {
        _.each(members.namespaces, function (v) {
            nav.namespaces.push({
                ...
                longname: v.longname,
                name: v.name,
                title: v.description || v.longname
            });
        });
    }
...
exports.publish = (taffyData, opts, tutorials) => {
    ...
    generate('Home',
        ...
        ).concat(files), indexUrl, null, false);
    ...
    Object.keys(helper.longnameToUrl).forEach(longname => {
        ...
        if (myClasses.length) {
            // calss不显示title
            generate(`Class: ${myClasses[0].name}`, myClasses, helper.longnameToUrl[longname], null, false);
        }

        if (myNamespaces.length) {
            // 修改title为中文描述
            generate(`${myNamespaces[0].description || myNamespaces[0].name}`, myNamespaces, helper.longnameToUrl[longname]);
        }
